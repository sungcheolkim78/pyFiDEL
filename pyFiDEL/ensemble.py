...
class FiDEL(object):
    def __init__(self, predictions: np.array, method_names: list = None):
        ...
        for i in range(self.n_methods):
            self.rank_matrix[:, i] = [index for e, index in sorted(zip(predictions[:, i], np.linspace(1, self.n_samples, num=self.n_samples)))]
    ...
    def calculate_performance(self, y: list, method: str = 'FiDEL', alpha: float = 1):
        ...
        self.df['AUC'] = [auc_rank(self.predictions[:, i], self.y) for i in range(self.n_methods)]
        self.df['beta'] = 1.
        self.df['mu'] = 0.
        self.df['r_star'] = 0.
        ...
        self.logit_matrix = np.power(self.df['beta'].values, alpha) * (self.df['r_star'].values - self.rank_matrix)
