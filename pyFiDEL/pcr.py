...
class PCR(object):
    ...
    def auc(self):
        ...
        return np.abs(np.sum(rank * self.pcr) / N1 - np.sum(rank * (1. - self.pcr)) / N2) / N + .5
    ...
    def auprc(self):
        ...
        return .5 * self.rho * (1. + np.sum(prec[1: (N - 2)] * prec[2: (N - 1)]) / (N * self.rho * self.rho))

    def build_metric(self) -> (pd.DataFrame, dict):
        ...
        self.df['fpr'] = np.cumsum(1. - self.pcr) / N2
        self.df['bac'] = .5 * (self.df['tpr'] + 1. - self.df['fpr'])
