...
def Pxy_int(beta: float, mu: float, rho: float, resol: float = 1E-6) -> float:
    ...
    print(f'... Pxy integral calculation with resolution = {resol}')

    p_table = fermi_b(np.linspace(0, 1, num=int(1. / resol)), b=beta, m=mu, normalized=False)

    A = p_table
    B = np.linspace(len(p_table), 1, num=len(p_table)) - np.cumsum(p_table[::-1])[::-1]

    ans = np.sum(A * B) - np.sum(A * (1. - A))
    ans = ans * resol * resol / (rho * (1. - rho))
...
def Pxxy_int(beta: float, mu: float, rho: float, resol: float = 1E-6) -> float:
    ...
    print(f'... Pxxy integral calculation with resolution = {resol}')

    p_table = fermi_b(np.linspace(0, 1, num=int(1. / resol)), b=beta, m=mu, normalized=False)

    A = 2. * p_table * np.cumsum(p_table) - p_table * p_table
    B = np.linspace(len(p_table), 1, num=len(p_table)) - np.cumsum(p_table[::-1])[::-1]

    ans = np.sum(A * B) - np.sum(p_table * p_table * (1. - p_table)) - np.sum(p_table * p_table) - 2. * np.sum(p_table * (1. - p_table))
    ans = ans * resol * resol * resol / (rho * rho * (1. - rho))
...
def Pxyy_int(beta: float, mu: float, rho: float, resol: float = 1E-6) -> float:
    ...
    A = p_table
    B = np.cumsum((1. - p_table)[::-1])[::-1] ** 2

    ans = np.sum(A * B) - np.sum(p_table * (1. - p_table) * (1. - p_table)) - np.sum((1. - p_table) ** 2) - 2. * np.sum(p_table * (1. - p_table))
    ans = ans * resol * resol * resol / (rho * (1. - rho) * (1. - rho))
